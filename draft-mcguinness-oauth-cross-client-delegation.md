...
Presenter Delegation {{I-D.mcguinness-oauth-presenter-delegation}} provides that transfer: the Initiator, holding the key bound in the Identity Assertion's `cnf` claim, signs an attenuated delegation to the Delegate's key, and the IdP validates the chain from the assertion's confirmation key to the key the Delegate proves possession of.  A deployment that combines key binding with this profile MAY use Presenter Delegation as the presenter-transition mechanism; its delegation chain is also an IdP-verifiable per-assertion artifact and an authenticated handoff, and so raises the exchange to the assertion-bound operation of {{authorization-model}}.  {{composition-pd}} defines the normative binding between the two documents.

## Composition with Presenter Delegation {#composition-pd}

A deployment MAY compose this profile with Presenter Delegation {{I-D.mcguinness-oauth-presenter-delegation}} to accept a key-bound Identity Assertion, which the base profile excludes ({{id-token-profile}}).  When it does, this section supplies the profile-specific definitions that {{I-D.mcguinness-oauth-presenter-delegation}} requires a consuming profile to provide.  This profile opts in to presenter delegation only for a key-bound Identity Assertion presented at Token Exchange under this profile; the IdP MUST NOT accept a Presenter Delegation Assertion (PDA) for any other purpose.  The CCDR-authorized audience exception of {{processing}} continues to apply unchanged: the Delegate is still not the assertion's audience, the CCDR still authorizes the pairing, and Presenter Delegation additionally continues proof of possession from the Initiator's key to the Delegate's.

Verifier and proof of possession:
: The Verifier is the IdP.  Every PDA's `aud` MUST contain the IdP's issuer identifier.  The accepted proof-of-possession method is DPoP {{RFC9449}}: the Delegate MUST present a DPoP proof of the outermost Recipient key on the Token Exchange request.

Confirmation-key reduction:
: The Source Token is the Initiator's key-bound Identity Assertion.  Its confirmation key is reduced to a JWK SHA-256 Thumbprint as specified in {{I-D.mcguinness-oauth-presenter-delegation}}.  For an assertion key-bound per OpenID Connect Key Binding {{OpenID.KeyBinding}}, whose `cnf` carries a `jwk`, that thumbprint is the JWK SHA-256 Thumbprint of the `jwk`.

Party identity:
: Presenter Delegation roots the chain in the Source Token's confirmation key ({{I-D.mcguinness-oauth-presenter-delegation}}), and step 2 of {{processing-steps}} resolves that assertion to the Initiator.  Together these map the chain's root signing key to the Initiator as a validated actor identity, which is what {{I-D.mcguinness-oauth-presenter-delegation}} requires before the Initiator may be recorded as a prior actor.  The party proving possession of the outermost Recipient key MUST be the client authenticated for the request, that is, the Delegate; the IdP confirms this because the Delegate authenticates as its client, presents its `actor_token`, and proves the Recipient key on the same request.  Intermediate keys in a chain are not actor identities and MUST NOT be recorded as prior actors.

Request-constraint mapping:
: The `pda_constraints` members `audience`, `resource`, `scope`, and `authorization_details` bound the correspondingly named Token Exchange request values.  Each such value in the resulting authorization MUST lie within the corresponding `pda_constraints` member, in addition to being authorized by the CCDR, `may_act`, and exchange-time policy.

Combination:
: A valid Delegation Chain is conjunctive with the CCDR, `may_act` when present, Delegate client and actor authentication, and exchange-time policy ({{authorization-model}}); it substitutes for none of them, and none of them overrides its constraints.  Because the Initiator's confirmation key cryptographically authorized the handoff, the IdP MAY record the Initiator as a nested prior actor ({{issued-token}}); the chain is the additional mechanism that section otherwise requires.
